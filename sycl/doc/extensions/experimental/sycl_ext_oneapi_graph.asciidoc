...
Whether native recording is in effect for a given graph can be queried with the
`info::graph::recording_mode` <<graph-info-descriptors, information
descriptor>>.
...
A `command_graph` that is received from outside the code holding it — across a
library boundary, or from a factory function — carries characteristics which
determine whether a given operation on it will succeed. The information
descriptors in the `info::graph` namespace make those characteristics
queryable through the `get_info` member function of `command_graph`.

These descriptors report the graph's *effective state*, not the contents of the
`property_list` the graph was constructed or finalized with. A descriptor may
therefore report a characteristic that was never requested through a property,
if the implementation enabled it by some other means.

`info::graph::recording_mode` is valid in both the `graph_state::modifiable`
and `graph_state::executable` states. `info::graph::updatable` is valid only in
the `graph_state::executable` state.

The `graph_recording_mode` enumerators have the following meaning:
...
* `graph_recording_mode::native` indicates that graph capture is delegated to
  the backend, and that the restrictions listed in
  <<native-recording-limitations, Limitations of Native Recording>> apply.
...
_Constraints:_ This member function is only available when the `command_graph`
state is `graph_state::modifiable`. `Param` must be one of the
<<graph-info-descriptors, graph information descriptors>> that is valid in the
`graph_state::modifiable` state, that is, `info::graph::recording_mode`.

_Returns:_ Information about the graph as described by `Param`.

`info::graph::recording_mode` returns `graph_recording_mode::native` if native
recording is in effect for this graph, whether it was enabled by passing
`property::graph::enable_native_recording` at construction or by setting
`SYCL_GRAPH_FORCE_NATIVE_RECORDING=1`. Otherwise it returns
`graph_recording_mode::runtime`.

[NOTE]
====
Example: a consumer of a graph produced elsewhere checking whether it may
inspect the graph's nodes:

[source,c++]
----
namespace exp_ext = sycl::ext::oneapi::experimental;

void consume(const exp_ext::command_graph<exp_ext::graph_state::modifiable>& g) {
  if (g.get_info<exp_ext::info::graph::recording_mode>() ==
...
_Constraints:_ This member function is only available when the `command_graph`
state is `graph_state::executable`. `Param` must be one of the
<<graph-info-descriptors, graph information descriptors>> that is valid in the
`graph_state::executable` state, that is, `info::graph::recording_mode` or
`info::graph::updatable`.

_Returns:_ Information about the graph as described by `Param`.

`info::graph::recording_mode` returns `graph_recording_mode::native` if the
modifiable graph this executable graph was finalized from used native
recording, and `graph_recording_mode::runtime` otherwise.

`info::graph::updatable` returns `true` if `property::graph::updatable` was
passed to the `finalize` call that created this executable graph, and `false`
otherwise. When it returns `false`, the `update` member functions throw an
`exception` with error code `invalid`.

[NOTE]
====
Example: a consumer of an executable graph produced elsewhere checking whether
it may update the graph:

[source,c++]
----
namespace exp_ext = sycl::ext::oneapi::experimental;

void consume(exp_ext::command_graph<exp_ext::graph_state::executable>& g,
             const std::vector<exp_ext::node>& newNodes) {
  if (g.get_info<exp_ext::info::graph::updatable>()) {
    g.update(newNodes);
  }
}
----
...
will throw an exception if attempted. A consumer holding a graph it did not create can
determine whether these restrictions apply by querying the
`info::graph::recording_mode` <<graph-info-descriptors, information
descriptor>>, rather than probing with exceptions.
...
|3|2026-08-27|Adam Fidel
|Add graph information descriptors: `info::graph::recording_mode` and
`info::graph::updatable`, queried via `command_graph::get_info()`
