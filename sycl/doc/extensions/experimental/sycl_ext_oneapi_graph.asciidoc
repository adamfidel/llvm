...
A `command_graph` that is declared at one call site and used in a different call site does not have access to information used to
instruct its creation, such as whether it is updatable or uses native recording. The `get_info` method can be used to obtain information
descriptors in the `info::graph` namespace to query such information.

These descriptors report the graph's effective state, which may differ from the contents of the
`property_list` the graph was constructed or finalized with.
...
recording is in effect for this graph. Otherwise it returns
`graph_recording_mode::runtime`.

`info::graph::updatable` returns `true` if `property::graph::updatable` was
passed to the `finalize` call that created the executable graph. This is only available when the `command_graph`
