...
|3     |Add `command_graph::get_info()` and the `info::graph` information descriptors.
|===
...
==== Graph Information Descriptors [[graph-info-descriptors]]

Characteristics of a `command_graph` object can be queried with the
`command_graph::get_info()` member function, using the information descriptors
in the `info::graph` namespace.
...
struct cycle_checks_enabled;
struct buffers_allowed;
struct updatable;
...
These descriptors report the *effective state* of the graph object, which is
not necessarily the same as the contents of the `property_list` the graph was
constructed or finalized with. An implementation may enable a characteristic by
a means other than a property, in which case the corresponding descriptor
reports that characteristic as enabled.

Each descriptor is only valid in a subset of the graph states, as given in the
table below. Querying a descriptor in a state in which it is not valid is
ill-formed.

Table {counter: tableNumber}. Graph information descriptors.
[%header,cols="2,2,1,1,4"]
|===
| Descriptor | `return_type` | Valid in _modifiable_ | Valid in _executable_ | Returns

| `info::graph::recording_mode`
| `graph_recording_mode`
| yes
| yes
| `graph_recording_mode::native` if native recording is in effect for this
  graph, otherwise `graph_recording_mode::runtime`. Native recording is in
  effect if the graph was created with
  `property::graph::enable_native_recording`, or if it was enabled by any other
  implementation-defined means, such as setting
  `SYCL_GRAPH_FORCE_NATIVE_RECORDING=1` in {dpcpp}. For an executable graph,
  the value reported is that of the modifiable graph it was finalized from.

  `graph_recording_mode::runtime` indicates that the SYCL runtime owns the
  graph's node model. It is the value reported for any graph that does not use
  native recording, including a graph built exclusively through the explicit
  graph-building API and never used for queue recording.

| `info::graph::cycle_checks_enabled`
| `bool`
| yes
| no
| `false` if the graph was created with `property::graph::no_cycle_check`,
  otherwise `true`. Note that the sense of this descriptor is the inverse of
  that of the property: it reports whether cycle checks are performed, not
  whether they are disabled.

| `info::graph::buffers_allowed`
| `bool`
| yes
| no
| `true` if the graph was created with
  `property::graph::assume_buffer_outlives_graph`, otherwise `false`.

| `info::graph::updatable`
| `bool`
| no
| yes
| `true` if the graph was finalized with `property::graph::updatable`,
  otherwise `false`.

| `info::graph::profiling_enabled`
| `bool`
| no
| yes
| `true` if the graph was finalized with `property::graph::enable_profiling`,
  otherwise `false`.
...
Whether native recording is in effect for a given graph object can be queried
with the `info::graph::recording_mode`
<<graph-info-descriptors, graph information descriptor>>. That query reports the
effective state of the graph, and so also accounts for native recording enabled
by means other than this property.
...
_Constraints:_ `Param` must be one of the
<<graph-info-descriptors, graph information descriptors>>, and must be valid in
the state of this `command_graph` as given by the descriptor table. This member
function is available in both graph states.

_Returns:_ Information about the graph, as described in the
<<graph-info-descriptors, graph information descriptor>> table.

[NOTE]
====
Example: a consumer receiving a graph from a library boundary branches on the
characteristics of the graph rather than probing them with exceptions:

[source,c++]
----
namespace exp_ext = sycl::ext::oneapi::experimental;

void consume(const exp_ext::command_graph<exp_ext::graph_state::modifiable> &graph) {
  if (graph.get_info<exp_ext::info::graph::recording_mode>() ==
      exp_ext::graph_recording_mode::runtime) {
    // Only safe when the SYCL runtime owns the node model.
    for (auto &node : graph.get_nodes()) { /* ... */ }
  }
}

void consume(const exp_ext::command_graph<exp_ext::graph_state::executable> &graph,
             const std::vector<exp_ext::node> &newNodes) {
  if (graph.get_info<exp_ext::info::graph::updatable>()) {
    graph.update(newNodes);
  }
}
----
====
...
exception if attempted. A consumer of a graph object it did not itself construct
can determine whether these limitations apply by querying the
`info::graph::recording_mode`
<<graph-info-descriptors, graph information descriptor>>, rather than by
catching the exceptions below.
...
|3|2026-08-06|Adam Fidel
|Add `command_graph::get_info()` and the `info::graph` information descriptors
