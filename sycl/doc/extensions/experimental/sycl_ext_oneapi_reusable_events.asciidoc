...
* For `enqueue_wait_event` and `enqueue_wait_events`, the nodes dependent on
  ...
  the earlier one. In the case of the first submission, its status is
  `info::event_command_status::complete` and no work is waited on.
* For `enqueue_signal_event`, `evt` is disassociated from any previous command
  each time the executable graph is submitted and its status becomes
