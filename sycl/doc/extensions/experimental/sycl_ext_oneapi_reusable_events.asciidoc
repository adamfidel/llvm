...
cross the boundary of the graph which is being recorded, where a graph is
defined by the link:../experimental/sycl_ext_oneapi_graph.asciidoc[
...
the graph, and it is re-evaluated each time the executable graph is submitted:

* For `enqueue_wait_event` and `enqueue_wait_events`, the nodes dependent on
  the wait event(s) cannot begin execution until the commands associated with `evt` or
  `evts` at the time the executable graph is submitted have completed.
  If the event is not associated with a new signal between two submissions of
  the executable graph, the later submission waits on the same association as
  the earlier one. If no signal has ever been associated with the event before
  the first submission, its initial status is
  `info::event_command_status::complete` and no work is waited on.
* For `enqueue_signal_event`, each submission of the executable graph
  immediately disassociates `evt` from any previous command and sets its status
  to `info::event_command_status::submitted`. Its status becomes
  `info::event_command_status::complete` once all nodes on which that node
  depends have completed.
  A command submitted to another queue or a host wait on `evt` therefore
  synchronizes with the most recent submission of the executable graph.
* It is valid for an application to synchronize between two executable graphs
  by recording an `enqueue_signal_event` in one graph that is later waited
  on by an `enqueue_wait_event` or `enqueue_wait_events` in the second graph.

This property is only supported for graphs which were created with the
`property::graph::enable_native_recording` property.

_Throws:_

 * An `exception` with the `errc::runtime` error code if the `graph_external`
   property has the value `true` and `q` is not in the
   `queue_state::recording` state.
 * An `exception` with the `errc::feature_not_supported` error code if the
   `graph_external` property has the value `true` and the graph being recorded
   was not created with the `property::graph::enable_native_recording`
   property.
...
int main() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    q_graph.ext_oneapi_graph(exec);

    // Waits for the graph's "event signal" node, not for the whole graph.
    graph_event.wait();
  }
}
----
