...
If this property is not used, then reusable event synchronization is subject
to the 
link:../experimental/sycl_ext_oneapi_graph.asciidoc#event-limitations[
Event Limitations] section of the graph specification.

The `graph_external` property enables the event wait and signal operations
described below to cross this boundary.
 ...
 * An `exception` with an implementation defined error code if the `graph_external`
