...
* `enqueue_wait_event` and `enqueue_wait_events` synchronize on events signaled
   prior to graph submission. The graph node blocks until the provided event(s)
   transition to `info::event_command_status::complete` status.
* `enqueue_signal_event` defines a graph node that enables host or device
   ...
   to state `info::event_command_status::submitted` during graph submission
   and transitions to `info::event_command_status::complete` after device
   completion during graph replay.
