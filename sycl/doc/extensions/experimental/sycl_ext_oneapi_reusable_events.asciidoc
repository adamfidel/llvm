...
*  At graph submission, `enqueue_wait_event` and `enqueue_wait_events` select
   each event's latest signal for synchronization. During replay, the node
   waits until those signals complete.
* `enqueue_signal_event` signals an event that can be synchronized by the host or device
   outside the graph. The event becomes `info::event_command_status::submitted`
