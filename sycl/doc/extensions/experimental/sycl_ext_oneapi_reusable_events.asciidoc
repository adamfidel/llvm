...
When recorded in a `command_graph` without `graph_external{true}`, these
operations may only synchronize with work in that graph.
...
=== New property for enqueuing event operations [[graph-external-property]]
...
The `graph_external` property lets reusable event waits and signals recorded
in a `command_graph` to synchronize with work outside it. `command_graph` is defined by the
...
*  At graph submission, the node defined by `enqueue_wait_event` or `enqueue_wait_events`
   selects each recorded event's latest signal for synchronization. During replay, it
   waits until those signals complete.
* `enqueue_signal_event` signals the captured event that can be synchronized by the host or device
   outside the graph. The recorded event becomes `info::event_command_status::submitted`
   at graph submission and `info::event_command_status::complete` when the
   signal runs during replay.
