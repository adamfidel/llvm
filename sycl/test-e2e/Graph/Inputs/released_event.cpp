// Test that cross-queue dependencies recorded into a graph survive the
// destruction of the events that expressed them. Two fork-joins are recorded,
// one using the events returned by submission and one using reusable events,
// and every event leaves scope before the graph is finalized. The graph is then
// replayed, so the recorded dependencies are all that remain to order the
// kernels.
...
int main() {
  ...
  // Only Queue1 is recorded explicitly. Queue2 is pulled into the recording by
  // the dependencies taken on it below.
