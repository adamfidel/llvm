...
///////////////////////////////////////////////////////////////////////////////
/// @brief Intercept function for urCommandBufferAppendHostTaskExp
__urdlllocal ur_result_t UR_APICALL urCommandBufferAppendHostTaskExp(
    /// [in] Handle of the command-buffer object.
    ur_exp_command_buffer_handle_t hCommandBuffer,
    /// [in] Host task callback function. Must not call any UR functions.
    ur_exp_host_task_function_t pfnHostTask,
    ...
    /// command-buffer is updatable.
    ur_exp_command_buffer_command_handle_t *phCommand) {
  auto pfnAppendHostTaskExp =
      getContext()->urDdiTable.CommandBufferExp.pfnAppendHostTaskExp;

  if (nullptr == pfnAppendHostTaskExp)
    return UR_RESULT_ERROR_UNSUPPORTED_FEATURE;

  ur_command_buffer_append_host_task_exp_params_t params = {
      &hCommandBuffer,
      &pfnHostTask,
      &pData,
      &pProperties,
      ...
      &phCommand};
  uint64_t instance = getContext()->notify_begin(
      UR_FUNCTION_COMMAND_BUFFER_APPEND_HOST_TASK_EXP,
      "urCommandBufferAppendHostTaskExp", &params);

  auto &logger = getContext()->logger;
  UR_LOG_L(logger, INFO, "   ---> urCommandBufferAppendHostTaskExp\n");

  ur_result_t result = pfnAppendHostTaskExp(
      hCommandBuffer, pfnHostTask, pData, pProperties, numSyncPointsInWaitList,
      pSyncPointWaitList, numEventsInWaitList, phEventWaitList, pSyncPoint,
      phEvent, phCommand);

  getContext()->notify_end(UR_FUNCTION_COMMAND_BUFFER_APPEND_HOST_TASK_EXP,
                           "urCommandBufferAppendHostTaskExp", &params, &result,
                           instance);

  if (logger.getLevel() <= UR_LOGGER_LEVEL_INFO) {
    std::ostringstream args_str;
    ur::extras::printFunctionParams(
        args_str, UR_FUNCTION_COMMAND_BUFFER_APPEND_HOST_TASK_EXP, &params);
    UR_LOG_L(logger, INFO,
             "   <--- urCommandBufferAppendHostTaskExp({}) -> {};\n",
             args_str.str(), result);
  }
...
__urdlllocal ur_result_t UR_APICALL urGetCommandBufferExpProcAddrTable(
  ...
  dditable.pfnAppendHostTaskExp = pDdiTable->pfnAppendHostTaskExp;
  pDdiTable->pfnAppendHostTaskExp =
      ur_tracing_layer::urCommandBufferAppendHostTaskExp;
