...
__urdlllocal ur_result_t UR_APICALL urCommandBufferAppendHostTaskExp(
  ...
  auto *pfnAppendHostTaskExp = dditable->CommandBufferExp.pfnAppendHostTaskExp;
  if (nullptr == pfnAppendHostTaskExp)
    return UR_RESULT_ERROR_UNINITIALIZED;

  // forward to device-platform
  return pfnAppendHostTaskExp(hCommandBuffer, pfnHostTask, pData, pProperties,
                              numSyncPointsInWaitList, pSyncPointWaitList,
                              numEventsInWaitList, phEventWaitList, pSyncPoint,
                              phEvent, phCommand);
}
  ...
  if (UR_RESULT_SUCCESS == result) {
    if (ur_loader::getContext()->platforms.size() != 1 ||
      ...
      pDdiTable->pfnAppendHostTaskExp =
          ur_loader::urCommandBufferAppendHostTaskExp;
